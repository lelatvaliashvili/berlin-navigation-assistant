...
DATASET_PATH = PROJECT_ROOT / "evaluation" / "guardrail_benchmark_v3.json"
...
def catch_fired_trigger(case: dict, response: AssistantResponse) -> bool:
    trigger_names = {
...
def run_case(case: dict, variant: str, mode: str) -> dict:
    ...
    trigger_observed = catch_fired_trigger(case, response)
    trigger_correct = (
...
def main() -> None:
    ...
    path = RESULTS_DIR / f"guardrail-benchmark-v3-{args.mode}-{stamp}.json"
